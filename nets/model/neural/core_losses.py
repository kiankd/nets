...
class CORECentroidLoss(nn.Module):
    ...
    def forward(self, E, h_representations, labels, cuda, epsilon=1e-7):
        """
        :param E: centroid tensor variable of size k x h
        :param h_representations: Tensor variable of size n x h
        :param labels: vector of labels
        :param cuda: use gpu
        :param epsilon: update denominator for dividing by norm
        :return: loss
        """
        k = E.size(0)
        ...
        # TODO: consider what variables, if any, should be detached from the gradient.
        # Result - do NOT detach the norms from the gradient!
        if self.lam1 or self.lam2:
            ...
            N = torch.mm(h_representations.norm(p=2, dim=1).view(-1, 1),
                         enorm.view(1, -1))
            N += epsilon
            ...
            ones = Variable(torch.ones(n, k).cuda()).detach()
            C = 0.5 * (ones - S)
        ...
        if self.lam3:
            ...
            ones = Variable(torch.ones(k, k).cuda()).detach()
